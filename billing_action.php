...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			{
				$status = '<button type="button" name="status_button" class="btn btn-warning btn-sm">İşlemde</button>';
				$print = '';
			}
			else
			{
				$status = '<button type="button" name="status_button" class="btn btn-success btn-sm">Tamamlanmış</button>';
 			}
			$sub_array[] = $status;
			$sub_array[] = '
			<div align="center">
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		$html .= '
			<tr>
				<td colspan="4" class="text-right"><b>Net tutar</b></td>
