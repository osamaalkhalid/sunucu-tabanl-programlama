...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = $row["table_capacity"] . ' Kişi';
			$status = '';
	...
	if($_POST["action"] == 'Edit')
	{
		...
		if($object->row_count() > 0)
		{
			$error = '<div class="alert alert-danger">Tablo Zaten Var</div>';
		}
		else
		{
			...
			$object->query = "
			UPDATE table_data 
			SET table_name = :table_name, table_capacity= :table_capacity
			WHERE table_id = '".$_POST['hidden_id']."'
			";

			$object->execute($data);

			$success = '<div class="alert alert-success">Tablo Güncellendi</div>';
		}
	...
	if($_POST["action"] == 'change_status')
	{
		...
		echo '<div class="alert alert-success">Tablo Durumu değişikliği '.$_POST['next_status'].'</div>';
	}
	...
	if($_POST["action"] == 'delete')
	{
		...
		echo '<div class="alert alert-success">Tablo Silindi</div>';
	}
}
